...
로컬 환경에서 동영상 전사, 편집, 자막 굽기를 할 수 있는 도구입니다.  
...
- **자막 굽기**: 굵은 고딕체·흰색 글자·검은 테두리로 자막을 동영상에 직접 굽기
...
Whisper는 **100개 언어**의 전사를 지원합니다. 앱의 언어 선택기에서는 14개 주요 언어 + 자동 감지를 선택할 수 있습니다.
...
> **설치 버전(권장)을 사용하시나요?**
> ZIP을 압축 해제하고 `EasyTranscribe.exe`를 더블 클릭하기만 하면 됩니다 — **아래 단계는 전혀 필요 없습니다.**
> 아래 안내는 **소스 코드에서 직접 실행하려는 개발자 전용**입니다.
...
출력 설정에서 **"자막 굽기"**를 체크하면 자막을 동영상에 직접 구울 수 있습니다.
...
**고민되면 그대로 두셔도 됩니다.** 기본값인 `large-v3-turbo`가 권장 모델입니다.

| 모델 | 정확도 | 속도 | 크기(대략) | 비고 |
|------|--------|------|------|------|
| **large-v3-turbo** | ◎ 최고에 근접 | 빠름 | 약 1.6 GB | **권장·기본값** |
| large-v3 | ◎ 최고 | 느림 | 약 2.9 GB | 정확도를 최우선할 때 |
| medium | ○ | 보통 | 약 1.5 GB | turbo와 크기가 비슷함 |
| small | △ | 빠름 | 약 0.5 GB | 훑어보기용 |
| base | △ | 매우 빠름 | 약 0.15 GB | 훑어보기용 |
| tiny | △ | 가장 빠름 | 약 0.08 GB | 동작 확인용 |

### 「large는 무겁지 않나요?」

**`large-v3-turbo`는 이름만큼 무겁지 않습니다.** medium과 크기가 비슷하면서 정확도는 large-v3에 가깝고, 속도도 더 빠릅니다. 「가볍게 쓰려고 medium을 고른다」는 선택은 거의 필요하지 않습니다.

정말 무거운 쪽은 `large-v3`(turbo의 약 2배)이며, 정확도를 최우선할 때만 고르시면 충분합니다.

### 다운로드에 대하여

모델 이름 앞에 **★**가 붙은 것은 이미 다운로드되어 바로 사용할 수 있습니다(환경에 따라 다릅니다). ★가 없는 모델은 처음 사용할 때 자동으로 다운로드됩니다.

### 처리 시간의 기준

- **Apple Silicon Mac**(M1 이상): mlx-whisper로 GPU를 사용해 동작하므로 상당히 빠릅니다
- **Windows / Intel Mac**: CPU로 동작하므로 동영상 길이의 **3～10배** 정도 걸립니다
