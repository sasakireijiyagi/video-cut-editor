...
> **通常のご利用はこのインストーラー版だけでOKです。**  
> このREADMEの下のほうにある「セットアップ」「pip install」「python editor.py」などの手順は、**ソースコードから実行したい開発者向け**のものです。インストーラー版をお使いの方はコマンドプロンプトやターミナルでの作業は一切不要です。
...
- **複数ファイルの文字起こし**：複数ファイルをキュー処理、実行中もファイル追加可能
...
Whisper は **100言語**の文字起こしに対応しています。アプリの言語選択では主要14言語＋自動検出を選べます。
...
> **インストーラー版をお使いの方（推奨）**  
> ZIPを**必ず展開（解凍）してから** `EasyTranscribe.exe` をダブルクリックしてください。  
> ZIP内のままexeを実行すると警告が出る場合があります。  
> exeを初めて開くとき「WindowsによってPCが保護されました」が出たら、「詳細情報」→「実行」をクリックしてください。

---

**以下はソースコードから実行する開発者向けの手順です。インストーラー版（EasyTranscribe.exe）をお使いの方は読む必要はありません。**
...
> **⚠️ PATH を設定した後は PC を再起動してください。** 再起動しないと変更が反映されません。

> **確認方法：** 再起動後、コマンドプロンプトで `ffmpeg -version` と入力してバージョンが表示されればOKです。
...
ZIPを展開したフォルダの中に `editor.py` があります。そのフォルダをコマンドプロンプトで開いて実行してください：

```bat
cd C:\Users\あなたのユーザー名\Downloads\EasyTranscribe
python editor.py
```
...
「🔬 研究用」「🎬 動画用」ボタンを押すと、用途に合わせた設定（間の記録・しきつめ・字幕焼き込み）が一括で選ばれます。  
あくまで出発点なので、押した後も各設定は自由に変更できます。

| ボタン | しきつめ | [間]を記録 | 字幕焼き込み |
|---|---|---|---|
| 🔬 研究用 | ON（[間]表示） | ON | OFF |
| 🎬 動画用 | OFF | OFF | ON |
...
「📂 複数ファイルを指定」ボタンから専用ダイアログを開きます。  
動画ファイルを複数まとめてウィンドウにドラッグ&ドロップしても同じダイアログが開きます。

1. 「ファイルを追加…」または「フォルダを追加…」で動画を登録
2. モデル・言語・[間]設定・TXT/CSV書き出しを確認
3. 「▶ 実行」で順番に処理開始
...
**「TXT出力」チェックボックス**（バッチでは「TXTでも書き出す」）をオンにして文字起こしすると、  
SRTに加えて時間つきのテキストファイル（.txt）も同じ場所に保存されます。

```
[00:00:01 - 00:00:05] こんにちは、今日はよろしくお願いします
[00:00:05 - 00:00:12] ではさっそく始めましょう
```

- 1エントリ1行・時間情報つきなので、**議事録づくりや、AIに読ませて要約させる用途**にそのまま使えます
- [間]の記録やしきつめモードを使った場合は、その適用後の内容が出力されます
- 文字起こし時だけでなく、ファイルバーの「**TXT書き出し**」ボタンを押せば、開いているSRT（編集中の内容も含む）をいつでもTXTとして書き出せます

---

## CSV書き出し（Excelで開ける表形式）

**「CSV出力」チェックボックス**（バッチでは「CSVでも書き出す」）をオンにして文字起こしすると、  
開始・終了・テキストが列に分かれたCSVファイルも同じ場所に保存されます。

- Excelでそのまま開いて並べ替え・フィルタができます
- 「メモ」列が空欄で用意されているので、**コーディング（分類・タグ付け）作業**にそのまま使えます
- 文字化け対策のためBOM付きUTF-8で保存されます
- 文字起こし時だけでなく、ファイルバーの「**CSV書き出し**」ボタンを押せば、開いているSRT（編集中の内容も含む）をいつでもCSVとして書き出せます
...
文字起こし実行前に **「しきつめ」チェックボックス** をオンにすると、  
発話と発話の間のすべての隙間（冒頭・末尾の無音も含む）にエントリが自動挿入されます。

- エントリの表示形式は **「空欄」** か **「[間]」** を選択できます
- 会話分析など、タイムライン上に空白区間を残さずすべての時間帯を記録したい場合に使います
- ジェットカットの誤操作防止にも有効です（「全選択して結合」しても隙間エントリが含まれるため、発話外の時間も保持されます）
- 複数ファイルの文字起こしでも同様に設定できます
...
**迷ったら、そのままで大丈夫です。** 初期設定の `large-v3-turbo` が推奨モデルです。

| モデル | 精度 | 速度 | サイズ（目安） | 備考 |
|--------|------|------|--------|------|
| **large-v3-turbo** | ◎ ほぼ最高 | 速い | 約 1.6 GB | **推奨・初期設定** |
| large-v3 | ◎ 最高 | 遅い | 約 2.9 GB | 精度を最優先したいとき |
| medium | ○ | 普通 | 約 1.5 GB | turbo とほぼ同じ大きさ |
| small | △ | 速い | 約 0.5 GB | 下読み・内容確認用 |
| base | △ | とても速い | 約 0.15 GB | 下読み用 |
| tiny | △ | 最速 | 約 0.08 GB | 動作確認用 |

### 「large は重いのでは？」について

**`large-v3-turbo` は名前ほど重くありません。** medium とほぼ同じ大きさで、精度は large-v3 に近く、しかも高速です。「軽くしたいから medium にする」という選び方をする必要はほとんどありません。

重いのは `large-v3` の方（turbo の約2倍）で、こちらは精度を最優先したいときだけ選べば十分です。

### ダウンロードについて

モデル名の先頭に **★** が付いているものは、すでにダウンロード済みですぐ使えます（お使いの環境によって変わります）。★のないモデルは、初回に使うとき自動でダウンロードされます。

### 処理時間の目安

- **Apple Silicon の Mac**（M1 以降）: GPU を使う mlx-whisper で動くため、かなり高速です
- **Windows / Intel Mac**: CPU で動作するため、**動画の長さの3〜10倍**程度かかります

---

## 注意事項

- 元の動画ファイルは変更されません（出力は常に別ファイル。同名ファイルが既にある場合は `_1`・`_2` と連番を付けて保存します）
