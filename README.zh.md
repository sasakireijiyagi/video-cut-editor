...
- **压制字幕**：将字幕直接压制到视频中（粗体黑体、白色文字、黑色边框）
...
Whisper 支持 **100种语言**的转录。应用语言选择器提供14种主要语言 + 自动检测。
...
> **使用安装版（推荐）？**
> 只需解压 ZIP 并双击 `EasyTranscribe.exe` — **无需执行以下任何步骤。**
> 以下说明**仅适用于希望从源代码运行的开发者**。
...
**拿不准就保持默认。** 默认的 `large-v3-turbo` 就是推荐模型。

| 模型 | 精度 | 速度 | 大小（约） | 备注 |
|------|------|------|------|------|
| **large-v3-turbo** | ◎ 接近最高 | 快 | 约 1.6 GB | **推荐·默认** |
| large-v3 | ◎ 最高 | 慢 | 约 2.9 GB | 精度优先时选用 |
| medium | ○ | 普通 | 约 1.5 GB | 与 turbo 大小相近 |
| small | △ | 快 | 约 0.5 GB | 用于快速通读 |
| base | △ | 很快 | 约 0.15 GB | 用于快速通读 |
| tiny | △ | 最快 | 约 0.08 GB | 用于验证运行 |

### 关于「large 会不会太重」

**`large-v3-turbo` 并不像名字那样重。** 它与 medium 大小相近，精度接近 large-v3，而且更快。几乎没有必要「为了轻量而改用 medium」。

真正重的是 `large-v3`（约为 turbo 的两倍），只在追求最高精度时选用即可。

### 关于下载

模型名前带 **★** 的表示已下载到本地，可直接使用（因设备而异）。没有 ★ 的模型会在首次使用时自动下载。

### 处理时间参考

- **Apple 芯片的 Mac**（M1 及以后）：通过 mlx-whisper 使用 GPU 运行，速度相当快
- **Windows / Intel Mac**：使用 CPU 运行，约为视频时长的 **3～10 倍**
